...
function setSlide($slider,index){
	...
	$slider.find('.slide').eq(index).addClass('active').css({'display':'block', 'background-size':'contain'});
...
function initSlider($slider){
	...
	$slider.find('.slide').each(function(index){
		$(this).css({'background':'url("'+$(this).data('background')+'") no-repeat', 'background-position':'center bottom', 'background-size':'contain',});
	});
